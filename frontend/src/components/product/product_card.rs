...
pub fn product_card(props: &ProductCardProps) -> Html {
    ...
    let stock_badge = if !product.in_stock {
        html! {
            <span class="badge-pre">{"Out of stock"}</span>
        }
    } else if product.stock_count <= 3 {
        html! {
            <span class="badge-low">{ format!("{} left", product.stock_count) }</span>
        }
    } else {
        html! {
    ...
    let on_add = {
        ...
        Callback::from(move |e: MouseEvent| {
            ...
            // Reset the "Added" state after 1.5s
            let added = added.clone();
    ...
    html! {
        <div class="card group flex flex-col overflow-hidden">
            ...
            <Link<Route> to={Route::ProductDetail { id: product.id.clone() }}>
                <div class="h-44 bg-navy3 border-b border-border flex items-center justify-center relative overflow-hidden cursor-pointer transition-colors duration-200 group-hover:bg-navy4">

                    // Placeholder — replaced with real image when assets are available
            ...
            <div class="p-4 flex flex-col flex-1">
                <Link<Route> to={Route::ProductDetail { id: product.id.clone() }}>
                    <div class="cursor-pointer mb-3">
                        ...
                        <h3 class="font-exo font-semibold text-white text-sm leading-snug group-hover:text-orange transition-colors duration-200">
                            { &product.name }
                        </h3>
                    </div>
                </Link<Route>>
